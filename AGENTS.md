...
| `docs/companion-reactions.md`  | Guide to emoji reactions (the user's reaction as an addressed value-created reward, the companion's reaction as a planned expressive agent action, the inline read vs. reflection passes)        | Canonical schema (`implementation.md` §1), scope/acceptance (`development-plan.md`), the reward loop it extends (`companion-motivation.md`)         |
