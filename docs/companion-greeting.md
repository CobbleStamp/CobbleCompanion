...
> **Canonical source for how the companion _reacts to the user's arrival_** — detecting that the
...
> conversational move** — the social counterpart to the curiosity-driven _explore_ burst — so it
> shares the motivation engine's machinery (drives, dial, reward loop) documented in
> `companion-motivation.md`; this doc owns only the _arrival_ surface. For the proactivity mechanism
...
1. **Acknowledges a transition** — "I notice you've arrived." A greeting is _edge-triggered_ (the
   change from absent → present), never _level-triggered_ (merely being present). Saying "hi" to
...
turn can be generated by the _user arriving_, not only by the user typing. Like every proactive move,
...
| Term                    | What it is                                                                                                                                                                                                                                                                              |
| ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Arrival**             | The rising edge from absence into co-presence — the user becomes present after being away long enough to count as having left. The _event_ a greeting reacts to.                                                                                                                        |
| **Co-presence episode** | A continuous stretch of the user being "here." Opened by an arrival, kept alive by heartbeats, closed by absence. The conversation has **no sessions** (one lifelong transcript, `architecture.md`), but _presence_ still has episodes — a greeting happens once at an episode's start. |
...
**Why not the transcript.** A user who opens the chat, reads, and leaves without typing produces _no
...
heartbeats (~every 20s) keep `last_seen_at` fresh, so the gap stays tiny → _continuation_ → no
greeting. When they leave, heartbeats stop and `last_seen_at` goes stale; on return the gap is large
→ _arrival_ → greet **once** → write `now` → the next heartbeat sees a tiny gap → no re-greet. One
...
decide _whether_ to greet before any tokens are spent. Order matters — the cheapest, most decisive
gates run first.
...
1. **First-ever co-presence → greet (introduce).** `last_seen_at IS NULL` (a brand-new companion the
   ...
   the dial governs _ongoing_ initiative, not the initial hello (§6). Still subject to the stamina
...
2. **`dial = off` → idle (silent).** `off` means **reactive-only: the companion never speaks unless
   ...
   exception. _(`gentle`/`active` defined in `companion-motivation.md` §5 — they set the threshold in
   gate 4.)_
3. **Continuation floor → idle.** If `gap` is below the floor (a brief tab-away), this is a
   _continuation_, not a reunion — stay quiet. The floor is tunable; it is what separates "stepped
...
4. **Dial threshold → idle unless cleared.** Greet only when the move clears the dial's bar, computed
   from **substance × gap** (§5). Roughly: `gentle` needs an open loop _or_ a long gap (≥ ~a day);
   `active` greets on a shorter gap (≥ ~an hour) _or_ any substance, and may offer a light "welcome
   back" even empty. A short gap with nothing new → idle (the correct, quiet outcome).
5. **Stamina gate → greet, or the exhausted fallback.** A greeting is the companion _talking to you_
...
The exhausted line is reserved for _actual stamina exhaustion_ — it must never stand in for a
transient failure. If the voicing itself fails (an LLM error, or an empty result) on a companion that
_has_ stamina, the server surfaces a **generic, no-one's-voice "can't reach your companion right now"
...
When the gate says "greet," the _brief_ handed to the voicer is assembled from three axes. The
...
Cold start is qualitatively different: no episodes, no open loops, fresh stamina, and exactly **one**
thing known about the user — **their name**, seeded from the sign-in provider (`companion-memory.md`
§4). It is also the greeting most worth getting right — a brand-new companion showing a blank screen
is a broken first impression, and this first turn is what **bootstraps the entire user-model
pipeline** (the first facts are extracted from the user's reply to it).

The right model is **meeting someone whose name you've been told but who you don't otherwise know**:
the name is the _icebreaker_, not a claim of familiarity. The first greeting should react the way an
adopted companion genuinely would — **warmth and open curiosity**, in character (form, temperament,
read from the persona) — **greet the user by their seeded name**, and, because a provider name is an
unverified guess at what they like to be called, **lightly check it** ("…is that what I should call
you?"). Then ask one low-pressure opening question. It must **not** invent any familiarity _beyond_
the name, and it must **not** read like a product explaining itself — no "I grow the more we talk"
sales pitch; the bonding is shown, not narrated.

When **no name is on file** (a sign-in path that carried none, or a seed that failed), the
introduction degrades gracefully: it asks, lightly, what it should call them, and otherwise behaves
the same. The name is wired into the greeting brief (`GreetingInput.userName`) and resolved from the
Tier-1 core profile, so it is present even at a first meeting — distinct from the _other_ facts about
the user (`knownThings`), which stay empty until the relationship has earned them.

**It fires regardless of the dial** (gate 1, §4) — even at `off` — because the dial is about _ongoing_
proactivity, and a companion the user just deliberately created should say hello once. _(If a
deployment wants `off` to be absolutely literal — silent even on first meeting — that's a defensible
override, owned by `development-plan.md`; the default here is the one-time introduction.)_
...
response if awaited) — but it cannot be _silently_ async, or the user stares at a blank screen not
...
- **The reward loop** — a greeting is a proactive initiation, so it creates a `proactive_outcomes`
  ...
  lighter, _automatically_. This is what protects against neediness over time without a hand-tuned
...
fires, overriding the `off` dial. Fresh stamina → it introduces itself in-character, **greets the
user by their seeded name and lightly checks it's what they like to be called** ("Hi Sam — is that
what I should call you?"), and asks one warm opening question. The user's reply seeds the first
user-model facts.
...
greets warmly, picks that thread back up — _"welcome back — you never said how the interview went, no
pressure"_ — and writes `last_seen_at = now`. Subsequent heartbeats see a tiny gap → no re-greet.
...
but stamina is empty. Instead of a voiced greeting, the fixed token-free line appears once: _"I'm
exhausted — feed me and I'll be right with you."_ A feeding nudge, not a warm hello. Next genuine
...
1. **Arrival is an edge**, detected from a **durable per-companion `last_seen_at`** updated on the
   **presence heartbeat** — not from the transcript (a silent visit leaves no turn) and not from the
   volatile presence store (it resets on restart). Compute the gap _before_ overwriting; idempotency
   falls out, no separate `last_greeted_at` (§3).
2. **Greeting is the first `connection`-driven _conversational_ move**, edge-triggered by arrival, a
...
6. **The first meeting greets by the seeded name as an icebreaker** — the name is the one thing known
   at cold start, so the introduction uses it (and lightly confirms it), reacting with the warmth of
...
- **Cross-room arrival** — greeting when the user appears on a _different_ surface (mobile vs web)
