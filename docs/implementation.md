...
and reads it to authorize the token-mint route; the decoupled **worker polls** it. Neither
side imports the other or `@cobble/core`.
...
| `DISCORD_SERVICE_CLIENT_ID`                   | The `service_registry.client_id` of the Discord worker. Read by **both** the API (enables + pins `POST /internal/discord/token` and the `discord.config.*` methods) and the worker (authenticates to the mint route). Empty (default) leaves the Discord surface off (`companion-discord.md` §9) |
| `DISCORD_TOKEN_KEY`                           | Base64 of a 32-byte AES-256-GCM key. Shared: the API encrypts a saved bot token with it (the `discord.config.*` methods), the worker decrypts. Empty disables the API's Discord methods. **Must stay stable** — rotating it strands tokens already in `discord_config`                           |
| `DISCORD_SERVICE_SECRET`                      | _(worker only)_ the secret half of the worker's service credential, presented to the mint route                                                                                                                                                                                                  |
| `DISCORD_WS_BASE_URL` / `DISCORD_MINT_URL`    | _(worker only)_ the `/ws` origin + the token-mint URL the worker connects to (loopback on the single-box deploy)                                                                                                                                                                                 |
| `DISCORD_POLL_INTERVAL_MS`                    | _(worker only)_ how often the worker polls `discord_config` for changes (default 20000)                                                                                                                                                                                                          |
