...
| **Growth Service (P5)** | Derives four-axis growth (knowledge, relationship, abilities checklist, emerged-personality card) + blended stage from substrate (`core/src/growth/`); owns the feeding economy (mechanism: `companion-economy.md`) | Growth is DERIVED, not scored; `companion_growth` stores only the idempotent high-water mark + earned **treats**. Recompute lazily on `GET /growth` and post-turn via a **Growth Runner** (mirrors consolidation); a genuine transition posts one canned **growth note** (announcer pattern) |
