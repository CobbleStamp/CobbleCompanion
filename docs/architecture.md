...
| **Identity Store**                                              | Companion "home" record (incl. `evolvedPersona` + evolution/consolidation cursors and the Tier-3 `user_persona` + its `user_model_updated_through_seq` cursor); JIT-provisions the `users` row from an auth claim — by `email` (Google) or `(service_client_id, external_id)` (`service_token`)                                                              | Source of truth surfaces load from                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **Service Registry**                                            | Server-to-server consumer credentials (`service_registry`, `core/src/identity/service-registry.ts`) — validates a `(client_id, secret)` pair (constant-time, per `secret_type`) for service-token callers; many secrets per consumer for overlap rotation                                                                                                    | Only consulted by the `ServiceTokenVerifier` at the auth boundary (§5, §8); admin CLI `pnpm --filter @cobble/db service`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| **Discord Surface (adapter)**                                   | A decoupled "bring-your-own-bot" surface (`packages/discord/`) — an always-on service hosting each user's Discord bot, bridging DMs to the companion                                                                                                                                                                                                          | A singleton sibling service (one gateway connection per user) that connects to `/ws` **as the real user** (a short-lived token minted by the api's internal `POST /internal/discord/token`) and speaks ONLY the public WS contract — it imports nothing from `@cobble/core`. `/summon` claims embodiment; DMs → `messages.send`; eight read-only slash commands; proposals as embed+buttons; proactive DMs; the bot token is encrypted at rest in `discord_config` (`@cobble/db`) by the api's `discord.config.*` methods; after a config write the api calls the adapter's internal reconcile endpoint so the bot (re)starts at once — **no poll, no cached config** (the adapter reads each field on demand, §2.1). Mechanism → `companion-discord.md`                                                                                                         |
...
/                      repo root
  ...
  packages/            TS monorepo (workspaces)
    core/              the companion (surface-agnostic) — invariant #1
      harness/         agent loop + extension hooks (§4); semantic + episodic recall (§4.3). Post-turn perception (affect sense+learn, user-fact capture, belief embedding + the serialize-by-key chains) lives in `post-turn-perception.ts`; the fire-and-forget tracker in `background-tasks.ts` — both injected into the loop class
      ...
      identity/        companion "home" model + store; the user-provisioning claim (email vs service); ServiceRegistry — server-to-server consumer credentials for service-token auth (§5, §8)
    ...
    api/               BFF / surface boundary (Fastify). The product runs over the **WS transport** (`ws/` — register · handshake · connection · dispatch · fencing · per-domain methods incl. the user-scoped `discord.config.*` Discord-settings methods; all req/resp + streaming + event push, §6). HTTP left: `/auth/config`, the filesystem upload sink `/uploads/local` (local `file` staging backend only), `/health`, the admin-only `/admin/queue` (Phase C2), the internal Discord token-mint route `POST /internal/discord/token` (off unless `DISCORD_SERVICE_CLIENT_ID` is set; companion-discord.md §9), and the SPA serve
      proposals/       transport-free approve-and-execute use case (`confirmProposal`) the `proposals.confirm` WS handler delegates to (claim → dispatch → audit-log → procedural + lead advance → outcome row)
      sources/         transport-free upload use case (`stageAndEnqueueFileSource`: validate → stage → enqueue → append) + `file-format.ts` (magic-byte/format gate + user-safe rejection messages) the `sources.file` WS handler delegates to
    ...
    web/               React web client over the permanent WebSocket (`src/api/ws.ts` transport singleton; snapshot + live establishment, §6); chat w/ citations + ingestion-status panel + approval cards + "moved rooms" re-claim, sources page, memory browser (incl. editable user-model profile/beliefs panel), usage badge; vitality meter + proactivity dial; growth view + kitchen; activity view (autonomous-initiative log); Discord settings panel (attach a bot token, pick the bound companion, show the `/link` code). The chat surface is decomposed under `src/pages/chat/` — `useEmbodimentSync` (standing channel + snapshot + reconnect/backoff + buffer + room-takeover), `chat-lines.ts` (React-free line model + pure merge/fold helpers), and the `ChatTranscript`/`ChatComposer` views
    discord/           the decoupled Discord surface adapter (companion-discord.md): the always-on gateway-manager service (one bot per user) + per-user bridge (a `/ws` client speaking only the public contract — `ws-client.ts` is a thin re-export of the shared `WsTransport` plus the Node `ws` socket factory in `connection.ts`); owner-lock + `/link`, summon/chat/read-only views, proposal embeds, proactive DMs; reads `discord_config` on demand (no poll/cache) and hosts an internal reconcile endpoint the api calls after a config write (§2.1). Imports `@cobble/db` + `@cobble/shared` only — never `@cobble/core`
    shared/            shared TS types / contracts + the canonical socket-agnostic `WsTransport` (`ws-transport.ts`: envelope demux, StreamQueue, embodiment-ready/superseded lifecycle, the WS error classes) — callers inject a `WsSocketFactory`; the discord adapter re-exports it with a Node `ws` factory
...
- **Service-to-service trust boundary** (service-token auth, routed by the `X-Service-Client-Id` header) — a trusted backend consumer
...
- **Event-log retention** — the durable `companion_events` log (§6) is **append-only with no
  ...
  contrast, _is_ reclaimed — the S3 bucket lifecycle rule, or the `file` backend's wired
