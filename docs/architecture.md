...
| **API / BFF**                                                   | Auth, the permanent-WS transport (request/response + turn streaming + event push), source intake (multipart upload)                                                                                                                                                                                                                                                                        | The only thing surfaces talk to                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| **WS Transport & Embodiment**                                   | The permanent WebSocket (`/ws`, `api/src/ws/`) carrying all request/response + events; claims `active_embodiment` (§6)                                                                                                                                                                                                                                                                         | One connection per companion (the "one room at a time" rule), claimed by a sortable **ULID `owner`**; a newer connection force-claims and the prior self-fences (handoff = "moving rooms", §5.2). Requests correlate by envelope `id`; turns stream as `{ id, stream }` chunks; events push as `{ event, data }`. The client establishes **snapshot + live**, merged **deduped by message id**                                                                                                                                                                                                                                                                                                                                                                                                   |
...
/                      repo root
  ...
  packages/            TS monorepo (workspaces)
    core/              the companion (surface-agnostic) — invariant #1
      ...
      embodiment/      live WS embodiment claim (`active_embodiment`) — one connection per companion, ULID owner, force-claim + heartbeat + TTL; claim-derived presence (§6, deliver-scalability.md §5.2)
    ...
    api/               BFF / surface boundary (Fastify). The product runs over the **WS transport** (`ws/` — register · handshake · connection · dispatch · fencing · per-domain methods; all req/resp + streaming + event push, §6). Only HTTP left: `/auth/config`, the multipart file upload, `/health`, the admin-only `/admin/queue` (Phase C2), and the SPA serve
...
- **Embodiment claim (one room at a time)** — a WS that names a companion is authenticated once at
  ...
  sortable **ULID `owner`** (`active_embodiment`). Companion-scoped methods re-check the lease before
