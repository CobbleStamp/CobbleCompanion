...
/                      repo root
  ...
  packages/            TS monorepo (workspaces)
    ...
    discord/           the decoupled Discord surface adapter (companion-discord.md): the always-on gateway-manager worker (one bot per user) + per-user bridge (a `/ws` client speaking only the public contract); owner-lock + `/link`, summon/chat/read-only views, proposal embeds, proactive DMs. Imports `@cobble/db` + `@cobble/shared` only — never `@cobble/core`
